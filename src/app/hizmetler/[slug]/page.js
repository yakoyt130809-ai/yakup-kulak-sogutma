...
export default async function ServicePage({ params }) {
  ...
  const problemSolutions = getServiceProblemSolutions(service.slug);
            ...
            <div className="mx-auto grid max-w-7xl gap-12 px-4 sm:px-6 lg:grid-cols-[1fr_0.85fr]">
              <div>
                ...
                <p className="mt-5 text-base leading-8 text-slate-600">
                  Bu hizmet, ticari soğutma sistemlerinde 30 yılı aşkın saha
                  tecrübesiyle verilir. {" "}
          ...
          <section className="py-16 sm:py-20">
            <div className="mx-auto max-w-7xl px-4 sm:px-6">
              <div className="max-w-3xl">
                <span className="text-sm font-semibold uppercase tracking-wider text-brand">Problem ve çözüm</span>
                <h2 className="mt-2 font-heading text-3xl font-extrabold text-[var(--navy)]">
                  {service.shortTitle} Gerektiren Arızalar ve Çözümler
                </h2>
                <p className="mt-4 leading-7 text-slate-600">
                  Aynı belirti farklı parçalardan kaynaklanabilir. Bu nedenle arıza, parça değişiminden önce sıcaklık, elektrik ve soğutma devresi ölçümleriyle doğrulanır.
                </p>
              </div>
              <div className="mt-10 grid gap-6 lg:grid-cols-3">
                {problemSolutions.map((item) => (
                  <section key={item.title} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <h3 className="font-heading text-xl font-bold text-[var(--navy)]">{item.title}</h3>
